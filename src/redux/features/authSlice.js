...
export const getUserFromServerThunk = () => async (dispatch, getState) => {
  dispatch(setLoadingValue(true));

  try {
    const response = await api.getUserFromServerAPI();
...
export const localLoginThunk =
  (loginFormDataObject) => async (dispatch, getState) => {
    dispatch(setLoadingValue(true));
...
export const localSignupThunk =
  (signupFormDataObject) => async (dispatch, getState) => {
    dispatch(setLoadingValue(true));

    const { email, password, cPassword, firstName, lastName } =
...
export const { setLoadingValue, setUserValue } = authSlice.actions;
